var colours = window.gradientSteps
  ? gradientSteps("#F3B845", "#DF4E76", 100)
  : [];
function getBrowser() {
  if (
...
async function getSnoozedTabs(ids) {
  ...
  var found = p.snoozed.filter(
    (s) => s.id && (ids.length ? ids.includes(s.id) : ids === s.id),
  );
  return found.length === 1 ? found[0] : found;
}
async function getOptions(keys) {
  var p = await new Promise((r) =>
    chrome.storage.local.get("snoozedOptions", r),
  );
...
async function getTabsInWindow(active) {
  ...
  var p = new Promise((r) =>
    chrome.tabs.query({ active: active, currentWindow: true }, r),
  );
  if (!active) return p;
  var tabs = await p;
  return tabs[0];
...
async function getAllWindows() {
  return new Promise((r) =>
    chrome.windows.getAll({ windowTypes: ["normal"] }, r),
  );
}
...
async function findTabAnywhere(url, tabDBId) {
  ...
  for (var wid of wins.map((w) => w.id)) {
    if (found) return;
    var tabs = await new Promise((r) =>
      chrome.tabs.query({ windowId: wid }, r),
    );
...
async function getStorageSize() {
  if (getBrowser() !== "firefox")
    return new Promise((r) => chrome.storage.local.getBytesInUse(r));
...
async function createWindow(tabId, incognito) {
  if (tabId)
    return new Promise((r) =>
      chrome.windows.create({ url: `/html/rise-and-shine.html#${tabId}` }, r),
    );
...
async function openExtensionTab(url) {
  ...
  if (extTabs.length === 1) {
    chrome.tabs.update(extTabs[0].id, { url, active: true });
  } else if (extTabs.length > 1) {
    var activeTab = extTabs.some((et) => et.active)
...
async function openTab(tab, windowId, automatic = false) {
  ...
  if (tab.incognito) {
    var w =
      ...
      (await createWindow(undefined, t.incognito));
    await new Promise((r) =>
      chrome.tabs.create(
        { url: tab.url, active: false, pinned: tab.pinned, windowId: w.id },
        r,
      ),
    );
...
async function openWindow(t, automatic = false) {
  var targetWindowID,
    currentWindow = await getTabsInWindow();
  if (
    currentWindow.length &&
    (currentWindow.filter(isDefault).length === currentWindow.length ||
  ...
  chrome.tabs.onUpdated.addListener(
    async function cleanTabsAfterLoad(id, state, title) {
...
async function editSnoozed(tabId, snoozeTime, duplicating) {
  ...
  ["startUp", "opened", "deleted", "repeat", "paused"].forEach(
    (prop) => delete t[prop],
  );
  ((t.wakeUpTime =
    snoozeTime === "startup"
...
async function snoozeWindow(snoozeTime, isASelection) {
  ...
  chrome.runtime.sendMessage({
    logOptions: [
      isASelection ? "selection" : "window",
      sleepyGroup,
      snoozeTime,
    ],
  });
  return isASelection
    ? { tabId: tabsInWindow.filter((t) => t.highlighted).map((t) => t.id) }
    : { windowId: tabsInWindow.find((w) => w.active).windowId };
}
...
async function snoozeRecurring(target, data) {
  ...
  var validTabs,
    activeTab,
    tabsInWindow = await getTabsInWindow();
  ...
  chrome.runtime.sendMessage({
    logOptions: [target, sleepyObj, sleepyObj.wakeUpTime],
  });
  if (target === "tab") return { tabId: activeTab.id };
  if (target === "window")
    return { windowId: validTabs.find((w) => w.active).windowId };
...
async function getChoices(which) {
  ...
  if (typeof config.morning === "number" || typeof config.evening === "number")
    config = upgradeSettings(config);
  var all = {
    startup: {
...
async function calculateNextSnoozeTime(data) {
  ...
  if (TYPE === "startup") {
    ...
    var days = [];
    if (data.weekly) {
      var thisWeek = data.weekly,
        nextWeek = data.weekly.map((day) => day + 7);
      days = nextWeek
        .concat(thisWeek)
        .map((day) =>
...
var isValid = (tabs) => {
  ...
  return (
    tabs.url &&
    validProtocols.includes(tabs.url.substring(0, tabs.url.indexOf(":")))
  );
};
...
var wrapInDiv = (attr, ...nodes) => {
  var div = Object.assign(
    ...
    typeof attr === "string" ? { className: attr } : attr,
  );
  div.append(...nodes);
  return div;
};
  ...
  object: (o) =>
    !o
      ...
      : Object.keys(o).reduce(
          (total, key) => calcObjectSize(key) + calcObjectSize(o[key]) + total,
          0,
        ),
};
...
var clipboard = (text) => {
  var el = Object.assign(document.createElement("textarea"), {
    innerText: text,
  });
...
var getHourFormat = (showZeros) =>
  HOUR_FORMAT && HOUR_FORMAT === 24 ? "HH:mm" : `h${showZeros ? ":mm" : ""} A`;
...
var showIconOnScroll = (_) => {
  ...
  document.addEventListener("scroll", (_) => {
    if (
      logo.classList.contains("hidden") &&
      window.pageYOffset > header.offsetHeight + header.offsetTop
    )
